...
function createBridge() {
    ...
    const FileUtils = Java.use("com.wuba.commons.file.FileUtils");
    bridge.readFileContent = function(filepath) {
    ...
    init: function (stage, parameters) {
        ...
        Interceptor.attach(Module.getExportByName(null, 'open'), {
            onEnter: function (args) {
                ...
                if (path.startsWith('/proc/') && path.indexOf('/task/') != -1) {
                    procList.push(path);
                }
            },
            onLeave: function (retval) {
                if (retval.toInt32() > 0) {
                    ...
                    let path = procList.shift();
                    if(path != undefined && path.startsWith('/proc/') && path.indexOf('/task/') != -1) {
                        // Java.perform(function() {
                        //     let fileContent = Shadow.Bridge.readFileContent(path)
